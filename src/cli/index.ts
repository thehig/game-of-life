...
const width = 24;
const height = 14;
let simulation = createDemoSimulation(width, height);
...
const renderPanelLines = (): string[] => {
  ...
  const faunaLine = details.fauna
    ? `${details.fauna.definition.name} (health ${details.fauna.state.health.toFixed(2)}, hunger ${details.fauna.state.hunger.toFixed(2)})`
    : "None";
